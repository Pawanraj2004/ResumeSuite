...
const TableRowHeader = ({ children }: { children: React.ReactNode }) => (
  <tr className="bg-blue-200 text-blue-900">
    <th
      className="px-4 py-3 text-lg font-semibold text-left rounded-t-lg"
      scope="colgroup"
      colSpan={2}
    >
      {children}
...
const TableRow = ({
  ...
  <tr
    className={cx(
      "divide-x bg-white hover:bg-gray-100 transition-colors",
      className
    )}
  >
    <th
      className="px-4 py-3 font-medium text-gray-800 text-left rounded-l-lg"
      scope="row"
    >
      {label}
    </th>
    <td className="px-4 py-3 text-gray-700 rounded-r-lg">
      {typeof value === "string"
        ? value
        : value.map((x, idx) => (
            <Fragment key={idx}>
...
export const ResumeTable = ({ resume }: { resume: Resume }) => {
  ...
  return (
    <table className="mt-4 w-full border border-gray-300 rounded-lg text-sm text-gray-900 shadow-lg">
  ...
  <>
    <TableRowHeader>Achievements</TableRowHeader>
    {resume.achievements.map((achievement, idx) => (
      <Fragment key={idx}>
        <TableRow label="Title" value={achievement.title || ""} />
        <TableRow label="Date" value={achievement.date || ""} />
        <TableRow
          label="Descriptions"
          value={achievement.descriptions || []}
          className={
            resume.achievements.length - 1 !== 0 &&
            idx !== resume.achievements.length - 1 &&
            "!border-b-4"
          }
        />
      </Fragment>
